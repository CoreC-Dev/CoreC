...
CoreC 以 **MIT 许可证** 开源：允许商用、修改与再分发，但**必须保留版权与许可声明**，且作者**不承担任何担保与责任**（按 "AS IS" 提供）。
...
完整条款见仓库根目录的 [LICENSE](https://github.com/CoreC-Dev/CoreC/blob/main/LICENSE)。
...
> 其中 **EPL-2.0**（Eclipse Public License）为弱传染型许可证，仅要求对 `paho.mqtt.golang` 本身的修改开源，不影响以本核心构建的闭源/商用产品。
